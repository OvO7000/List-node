...
work.index = joi.object().keys({
    subType: joi.string().regex(/^[a-z0-9]{24}$/).required(),
    count: joi.number().integer().min(0).required()
})
work.del = joi.object().keys({
    id: joi.string().regex(/^[a-z0-9]{24}$/).required(),
})

sub.add = joi.object().keys({
...
sub.del = joi.object().keys({
    ids: joi.array().items(joi.string().regex(/^[a-z0-9]{24}$/).required()),
})


img.adds = joi.object().keys({
    ids: joi.array().items(joi.string().regex(/^[a-z0-9]{24}$/)).required()
})
img.del = joi.object().keys({
    id: joi.string().regex(/^[a-z0-9]{24}$/).required()
})
img.edit = joi.object().keys({
    id: joi.string().regex(/^[a-z0-9]{24}$/).required(),
    sub: joi.string().regex(/^[a-z0-9]{24}$/).required()
})
img.add = joi.object().keys({
    sub: joi.string().regex(/^[a-z0-9]{24}$/).required()
})
